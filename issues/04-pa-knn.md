...
1. Open `learning/pa-knn/kNN.py`. Confirm the file starts with `# /// script`
...
8. From the repo root run:
   ...
   ```bash
   uv run learning/pa-knn/kNN.py learning/resources/data/small.arff --k 3 --distance 1
   ```

   or `task knn`. Open `learning/pa-knn/output_knn.md` and check the matrix
   shape matches the classes in the data README.
9. Run once on `medium.arff` (`task knn DATA=learning/resources/data/medium.arff K=5`)
   so you see the cost. You do not need a polished writeup for wine yet.
   Do **not** scale features in this ticket.
10. On a `pa-knn/…` branch, commit the implementation and `output_knn.md`
    for `small.arff` (`feat(pa-knn): implement leave-one-out kNN`) and
...
- Working `learning/pa-knn/kNN.py` with class `KNN` and the methods above.
- `learning/pa-knn/output_knn.md` from a `small.arff` run (settings, time,
  tie-break rule, labeled confusion matrix).

## Acceptance criteria

- `task knn` and `task knn DATA=learning/resources/data/medium.arff K=5` run
  under `uv run` with no project venv.
- Leave-one-out excludes the query row.
- `--distance` selects Euclidean, Manhattan, or Minkowski.
- The class is named `KNN` so pa-scaled can do `import_pa('pa-knn').KNN`.
- Stretch is optional. Skipping `stretch_cosine` does not block pa-scaled.
