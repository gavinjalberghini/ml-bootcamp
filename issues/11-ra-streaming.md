# [ra-streaming] Data Flow and Sampling Contexts

You need: batch kNN (pa-knn through pa-ensemble). Every evaluation so far assumed a complete
file.

You will: learn batch vs stream and sampling so the later drift tickets are
not a surprise. You will not write stream code until pa-online.
...
5. Answer every heading in `learning/ra-streaming/answers.md`:
...
- Filled `learning/ra-streaming/answers.md`.

## Acceptance criteria

- Every heading has an answer.
- You can say, in one sentence, why pa-online cannot use leave-one-out on the
  whole stream.
- Stretch is optional. Skipping it does not block ra-hardware.

## Stretch goal (optional)

**Challenge:** In `learning/ra-streaming/answers.md` under “Stretch goal”, design a
