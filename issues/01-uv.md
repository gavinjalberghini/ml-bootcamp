...
- Run it with `uv run learning/pa-knn/kNN.py …`. The next ticket installs
  Task so `task knn` is the same command. uv reads **that file's** header,
  not `pyproject.toml`.
...
- Stretch is optional. Skipping it does not block [taskfile](02-taskfile.md).
