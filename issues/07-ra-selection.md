# [ra-selection] Model Selection and Validation

You need: pa-metrics done. You can report macro-F1. You have been using a single
`k` (often 3).

You will: learn why you must not pick `k` on the same number you publish.
pa-selection will hold out 20% of the rows, choose `k` on the rest, and score the
held-out slice.
...
6. Answer every heading in `learning/ra-selection/answers.md`:
...
- Filled `learning/ra-selection/answers.md`.

## Acceptance criteria

- Every heading has an answer.
- You describe the 80/20 split pa-selection will use, in your own words.
- Stretch is optional. Skipping it does not block pa-selection.

## Stretch goal (optional)

**Challenge:** In `learning/ra-selection/answers.md` under “Stretch goal”, sketch
nested validation in words (outer split for the number you publish, inner
split for choosing `k`). No code. pa-selection stays a single holdout.
