...
# dependencies = []
# ///
"""pa-scaled: subclass knn.KNN. Add --normalize and --task. Do not copy pa-knn."""
...
class ScaledKNN(knn.KNN):
    """pa-knn kNN plus per-query scaling and optional binary label recoding."""
    ...
    def recode_labels(self, labels, source_path: str):
        """binary: see the pa-scaled ticket. multiclass: return labels unchanged."""
        # sudo:
    ...
    def predict_one(self, query, pool_x, pool_y) -> str:
        ...
        raise NotImplementedError('scale, then knn.KNN.predict_one')
...
def parse_args():
    parser = argparse.ArgumentParser(description='pa-scaled normalized kNN')
