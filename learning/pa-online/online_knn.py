...
# dependencies = []
# ///
"""pa-online: prequential window. Predict with scaled.ScaledKNN.predict_one; score with pa-metrics."""
...
class OnlineKNN(scaled.ScaledKNN):
    ...
    def vote(self, neighbor_labels: list, neighbor_weights: list | None = None) -> str:
        # sudo:
        #   if weights are off: same majority as pa-knn
...
def parse_args():
    parser = argparse.ArgumentParser(description='pa-online online sliding-window kNN')
    ...
    Path(args.output).write_text(
        '\n'.join(
            [
                ...
                '## Discussion',
                '',
                '(window size, drift, weighted vs uniform — see the pa-online ticket)',
                '',
            ]
        )
    )
