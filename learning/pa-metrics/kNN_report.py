...
# ]
# ///
"""pa-metrics: subclass scaled.ScaledKNN. Add baseline, metrics, and plots.
...
class ReportingKNN(scaled.ScaledKNN):
    """Scaled kNN plus evaluation helpers used by pa-metrics, pa-ensemble, pa-online, and pa-selection."""
...
def parse_args():
    parser = argparse.ArgumentParser(description='pa-metrics kNN report')
    ...
    Path(args.output).write_text(
        '\n'.join(
            [
                ...
                '## Discussion',
                '',
                '(answer the pa-metrics closing questions)',
                '',
            ]
        )
    )
