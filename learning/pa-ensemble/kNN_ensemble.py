...
# dependencies = []
# ///
"""pa-ensemble: bag scaled.ScaledKNN members. Score with metrics.ReportingKNN.metrics_report."""
...
class EnsembleKNN(scaled.ScaledKNN):
...
def parse_args():
    parser = argparse.ArgumentParser(description='pa-ensemble kNN ensembles')
...
def main():
    ...
    reporter = metrics.ReportingKNN(k=args.k, distance=args.distance, p=args.p, normalize=args.normalize)
    ...
    Path(args.output).write_text(
        '\n'.join(
            [
                ...
                '## Discussion',
                '',
                '(answer the pa-ensemble closing questions)',
                '',
            ]
        )
    )
